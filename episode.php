    ...
    <div id="content">
        <div class="container">
            <div id="list" class="row">
                ...
                // $movieid ='37873';
                // $seasondid ='8158';
                while ($pageno <= $maxpageno && $pageno < 20) {
                    $ch = curl_init();
                    curl_setopt($ch, CURLOPT_URL, 'http://live.new4k.tv/stalker_portal/server/load.php?type=vod&action=get_ordered_list&category=' . $cat . '&movie_id=' . $movieid . '&season_id=' . $seasondid . '&episode_id=0&p=' . $pageno . '&sortby=added');
                    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
                    curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'GET');
                    curl_setopt($ch, CURLOPT_HTTPHEADER, array(
                        'User-Agent: Mozilla/5.0 (QtEmbedded; U; Linux; C) AppleWebKit/533.3 (KHTML, like Gecko) MAG200 stbapp ver: 2 rev: 250 Safari/533.3',
                    ...
                    if ($resp == 'Authorization failed.') {
                        ...
                        for ($x = 0; $x < sizeof($token1); $x++) {
                            ...
                            $cmd = $tempvar["id"];
                            echo '<div class="col-6 col-sm-4 col-md-3 col-lg-3 col-xl-2"><a href="seriesplay.php?c=';
